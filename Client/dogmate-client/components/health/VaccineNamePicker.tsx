...
type Props = {
  value: IsraelVaccineKey | null;
  onChange: (key: IsraelVaccineKey) => void;
  placeholder?: string;
  disabled?: boolean;
};

export default function VaccineNamePicker({
  value,
  onChange,
  placeholder = 'בחר שם חיסון / טיפול',
  disabled = false,
}: Props) {
  ...
  const displayLabel = useMemo(() => {
    if (!value) return '';
    return ISRAEL_VACCINE_OPTIONS.find((o) => o.key === value)?.label ?? '';
  }, [value]);
        ...
        <Pressable style={styles.overlay} onPress={closeModal}>
          ...
          >
            <ScrollView
              ...
              bounces={false}
            >
              {ISRAEL_VACCINE_OPTIONS.map((item) => {
